...
describe("mset", () => {
  it("should store a value with a specific ttl", async () => {
    ...
    await expect(sqliteCache.store.mget("foo", "foo2")).resolves.toStrictEqual(["bar", "bar2"]);
  });
  ...
  it("should store a value with a specific ttl from global", async () => {
    ...
    await expect(sqliteCacheTtl.store.mget("foo", "foo2")).resolves.toStrictEqual(["bar", "bar2"]);

    await sleep(500);

    await expect(sqliteCacheTtl.store.mget("foo", "foo2")).resolves.toStrictEqual([undefined, undefined]);
  });
  ...
  it("should store a value without ttl", async () => {
    ...
    await expect(sqliteCache.store.mget("foo", "foo2")).resolves.toStrictEqual(["baz", "baz2"]);
  });
...
describe("mget", () => {
  it("should retrieve a value for a given key", async () => {
    ...
    await expect(sqliteCache.store.mget("foo", "foo2")).resolves.toStrictEqual([value, value2]);
  });
  it("should return null when the key is invalid", () =>
    expect(sqliteCache.store.mget("invalidKey", "otherInvalidKey")).resolves.toStrictEqual([undefined, undefined]));
...
describe("ttl", () => {
  it("should retrieve ttl for a given key", async () => {
    ...
    await expect(sqliteCache.store.ttl("foo")).resolves.toBeGreaterThanOrEqual(ttl - 10);

    await sqliteCache.set("foo", "bar", 0);
    await expect(sqliteCache.store.ttl("foo")).resolves.toEqual(-1);
  });

  it("should retrieve ttl for an invalid key", () => expect(sqliteCache.store.ttl("invalidKey")).resolves.toEqual(-2));
...
describe("keys", () => {
  ...
  it("should return an array of all keys if called without a pattern", async () => {
    ...
    await expect(sqliteCache.store.keys("f*").then((x) => x.sort((a, b) => a.localeCompare(b)))).resolves.toStrictEqual(
      ["foo", "foo2", "foo3"],
    );
  });
...
describe("wrap function", () => {
  ...
  it("should work", async () => {
    ...
    await expect(sqliteCache.wrap("wrap-promise", () => getUser(id + 1))).resolves.toStrictEqual({ id });
  });
});
